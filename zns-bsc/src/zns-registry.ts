...
import { Domain, Account, Transfer, Registration, ExpiryExtended } from "../generated/schema"
...
export function handleMintedDomain(event: MintedDomainEvent): void {
  ...
  domain.labelhash = Bytes.fromByteArray(crypto.keccak256(Bytes.fromUTF8(domain.labelName!)))
...
export function handleRenewedDomain(event: RenewedDomainEvent): void {
  ...
  domain.save()
   ...
   let expiryExtendedId = event.transaction.hash.toHexString().concat('-').concat(event.logIndex.toString())
